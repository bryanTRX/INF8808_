...
export const VIZ_CATALOG: VizEntry[] = [
  {
    ...
    narrative: 'Before we compare individual tracks, we need a map of the musical world. Each line in this parallel-coordinates plot is a genre — danceability, energy, acousticness, valence, and popularity drawn as connected points across five axes. Genres that rise and fall together share a sonic personality. Sharp zigzags reveal what makes each genre unique.',
    insight: 'Look for clusters of lines moving in sync. That is evidence of genres that sound alike in the data, even when their names seem worlds apart.',
  ...
  },
  {
    ...
    narrative: 'Song length is one of the quietest conventions in music — listeners rarely notice it, but producers feel its pressure. These box plots show how long tracks run in each of the top 50 genres, from short punchy singles to sprawling album cuts. The dashed reference line marks the median duration for pop, our baseline for a typical track.',
    insight: 'Wide boxes mean a genre accommodates many different lengths. Narrow boxes suggest artists have converged on a standard formula.',
  ...
  },
  {
    ...
    insight: 'A positive bar means more of this feature tends to mean more popularity. Strength matters as much as direction — even weak correlations shape the average hit.',
    readingGuide: 'Start with Pearson, then switch to Spearman. Features that remain important in both views are the most reliable signals.',
  },
  {
    id: 5,
    anchor: 'viz-5',
    ...
    narrative: 'Tempo sets the pace and danceability captures how easy a track is to move to. Together they describe the physics of groove. These faceted scatter plots sample tracks from major genres, each panel with its own cloud of points and a regression line showing the local trend. Filter genres, adjust sample size, or search for a specific artist to see where they land.',
    insight: 'A steep upward slope means faster songs in that genre also tend to feel more danceable. Not every genre follows that rule.',
  ...
  },
  {
    ...
    narrative: 'Acousticness measures how unplugged a recording sounds — sparse guitars and room ambience score high while dense production scores low. This chapter pits Pop against Rock in a direct comparison, with regression lines and binned averages showing whether quieter, more organic sounds help or hurt popularity in each camp.',
    insight: 'If the regression slopes differ between Pop and Rock, the same sonic choice carries a different meaning depending on the genre.',
    readingGuide: 'Read the stat cards for Pearson r and mean popularity in low versus high acoustic tracks, then confirm the pattern in the scatter.',
  },
  {
    id: 7,
    anchor: 'viz-7',
    ...
    narrative: 'Radio formats, playlist attention spans, and streaming skip rates all raise the same question: is there an ideal song length? Tracks are grouped into 30-second intervals by duration and each bar\'s height shows the average popularity in that window. Scroll to the three to five minute band where hits often cluster.',
    insight: 'The headline answer updates live from the data — watch how the sweet spot shifts when you change the visible range.',
    readingGuide: 'Hover bars for exact averages, toggle song counts to see sample size, and scroll to the 3 to 5 minute range to compare it against the full catalog.',
  },
  {
    id: 8,
    anchor: 'viz-8',
    ...
    narrative: 'Not every popular song is popular in the same way. Genres are split into popularity tiers — from emerging tracks to chart dominators — then broken down by an audio feature you choose. The result is a stacked portrait of what each tier sounds like within a genre. Do breakthrough hits have more energy than long-tail catalog favorites?',
    ...
    readingGuide: 'Pick a feature, then read each stack left to right as popularity tiers rising from low to high within a genre.',
  },
  {
    id: 9,
    anchor: 'viz-9',
    ...
    narrative: 'Scatter plots show individual points. Heatmaps show concentration. These 50x50 density grids reveal where tracks pile up in feature space — bright cells are sonic common ground, dark corners are rare combinations. Marginal histograms along each axis show the underlying distributions. Switch between Energy/Loudness and Speechiness/Danceability to explore two different maps of the catalog.',
    insight: 'Hot spots are the default sound of the dataset. Points in the cool edges are the experiments and uncommon combinations.',
  ...
  },
  {
    ...
    insight: 'Radar shapes that overlap belong to artists with similar sonic identities. Diverging shapes reveal why two equally popular stars can sound so different from one another.',
